...
**Multi-tenancy:** Every data table (suppliers, customers, products, sales, purchases, payments, returns, ledger_entries) has a `userId` column. All API routes are protected by JWT auth middleware (`src/middleware/auth.ts`) that injects `userId` from the token. Every DB read and write is scoped to `userId` — each shop/user sees only their own isolated data.

**Features:**
- JWT-based authentication (login + register); every other route requires a valid token
