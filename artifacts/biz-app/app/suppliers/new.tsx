...
export default function NewSupplierScreen() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/suppliers`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ name, phone, email, address, gstNumber }) });
      if (!res.ok) throw new Error("Failed");
      qc.invalidateQueries({ queryKey: ["suppliers"] });
      router.back();
    } catch { Alert.alert("Failed to create supplier"); }
    setLoading(false);
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: bottomPad + 20 }]} keyboardShouldPersistTaps="handled">
        <FormField label="Name *" value={name} onChangeText={setName} placeholder="Supplier name" autoCapitalize="words" />
        ...
        <TouchableOpacity style={[styles.submitBtn, { backgroundColor: loading ? colors.primary + "80" : colors.primary }]} onPress={handleSubmit} disabled={loading} activeOpacity={0.85}>
          {loading ? <ActivityIndicator color="#FFF" /> : <><Feather name="check" size={20} color="#FFF" /><Text style={styles.submitText}>Save Supplier</Text></>}
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
